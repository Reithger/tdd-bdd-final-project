...
def list_products():
    ...
    if ref_category is not None:
        category = getattr(Category, ref_category.upper())
        categ = Product.find_by_category(category)
        temp = []
        for prod in products:
            if prod in categ:
                temp.append(prod)
        products = temp
    if ref_available is not None:
        available = Product.find_by_availability(ref_available in ["True", "true", "1"])
        temp = []
        for prod in products:
            if prod in available:
                temp.append(prod)
        products = temp
    out = []
    for prod in products:
